...
class Account:
    ...
    def find(options: dict = dict()):
        ...
        sql = "SELECT * FROM accounts WHERE email LIKE '%{}%' LIMIT {} OFFSET {}".format(
            query, limit, start)
    ...
    def save(self):
        ...
        cursor.execute(query, (self.email, self.first_name, self.last_name, bcrypt.hashpw(
            self.password.encode('utf-8'), bcrypt.gensalt()), self.role))
    ...
    def update(self):
        if (Account.is_email_exists(self.email)):
            raise DuplicateDataException('account')
